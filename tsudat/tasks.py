...
def run_tsudat_simulation(user, scenario_id):
    ...
    for result in output_json['results_max']:
        abstract = "This layer shows the maximum *flow depth/flow velocity/wave amplitude* from a 1 in *return_period* tsunami that was generated by a magnitude *mag* earthquake on the *source_zone* subduction zone. The tsunami amplitude at the 100 m water depth contour was *waveheight* and the tide height during the simulation was *tide* m. Elevation data used in this tsunami simulation includes *elevation input 1, elevation input 2....*."
        ...
        save(base, tif_file_path, user, overwrite = False, title="Dummy Title", abstract=abstract, keywords = []) 
        layers.append(base)
