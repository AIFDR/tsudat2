"""
Module to export the 'depth' value maximum values only on land:

    result = (stage - elevation) if elevation > 0.01 else NODATA
"""
...
def export_depthonland_max(name_in, name_out,
    ...
    for start_slice in xrange(0, number_of_points, block_size):
        ...
        # get 'depth', but only if elevation > 0.01
        res = num.where(elevation > 0.01, stage-elevation, NODATA_value)
    ...
    log.debug('Processed values for depthonland are in [%f, %f]'
